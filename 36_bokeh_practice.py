"""
Final version, adds an option to plot the month count.
    ...
    while not done:
        try:
            ...
            if operation == 1:
                ...
                print(get_month_count(read_json(file)))
            elif operation == 5:
                plot()
            elif operation == 6:
                print("Goodbye.")
...
def get_month_count(json):
    """
    Counts number of birthdays in each month and then prints results
    :param json: DATA not file. use read_json(file) to read a file if necessary
    """
    months = []
    for p in json["presidents"]:
        bday = p["birthday"].split()
        months.append(bday[0])
...
def plot():
    ...
    m_count = get_month_count(read_json(file))
    x_cats = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"]
    x = []
    y = []
    for item in m_count:
        x.append(item)
        y.append(m_count[item])

    p = figure(x_range=x_cats)
